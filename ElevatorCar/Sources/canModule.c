...
void initCan (){
    ...
    CANIDAR2 = (ACC_CODE(ELEVATOR_CAR_ID) >> 8) & 0xFF;	//|\ 16-bit Filter 1
	CANIDMR2 = (MASK_CODE(ELEVATOR_CAR_ID) >> 8) & 0xFF;	//| \__ Accepts Standard Data Frame CANMessage
	CANIDAR3 = ACC_CODE(ELEVATOR_CAR_ID) &0xFF;			//| / with ID 0x100
	CANIDMR3 = MASK_CODE(ELEVATOR_CAR_ID) & 0xFF;		    //|/
